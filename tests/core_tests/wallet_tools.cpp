...
void wallet_tools::process_transactions(std::vector<tools::wallet2*> wallets, const std::vector<test_event_entry>& events, const cryptonote::block& blk_head, block_tracker &bt, const boost::optional<crypto::hash>& blk_tail)
{
  map_hash2tx_t mtx;
...
void wallet_tools::process_transactions(std::vector<tools::wallet2*> wallets, const std::vector<const cryptonote::block*>& blockchain, const map_hash2tx_t & mtx, block_tracker &bt)
{
  ...
  for(ssize_t i = roffset; iters < ntrans && !abort; i += step, ++iters)
  {
    ...
    try {
      ...
      if (fnc_accept && !(fnc_accept.get())({.td=&td, .src=&src, .selected_idx=&selected_idx, .selected_kis=&selected_kis,
                                              .ntrans=ntrans, .iters=iters, .sum=sum, .cur_utxo=cur_utxo}, abort))
          continue;
